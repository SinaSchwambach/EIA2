...
(function (Abschluss) {
    ...
    let handOpponent = [];
    function main() {
        let numOfCards = prompt("Mit wie vielen Karten willst du spielen?");
    ...
    function drawCardsOpponent(_numCards) {
        for (let x = 0; x < _numCards; x++) {
            let random = Math.floor(Math.random() * cards.length);
            let getcard = cards.splice(random, 1)[0];
            console.log(getcard);
    ...
    function displayHandOpponent() {
        let handOpponentdiv = document.getElementById("handOpponent");
        document.getElementById("handOpponent").innerHTML = "";
        for (let o = 0; o < handOpponent.length; o++) {
            let span = document.createElement("span");
            let id = o.toString();
    ...
    function playCard(_selectedCard) {
        ...
        playCardOpponent(_selectedCard);
    }
    ...
    function compareselectedCardWithHandOpponent(_selectedCard, card2) {
        let x = _selectedCard.color.toLowerCase();
        let y = card2.color.toLowerCase();
        if (x < y) {
            return -1;
        }
    ...
    function playCardOpponent(_selectedCard) {
        let pile = document.getElementById("Ablage");
        pile.innerHTML = "";
        let span = document.createElement("span");
